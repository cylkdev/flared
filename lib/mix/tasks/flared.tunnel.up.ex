defmodule Mix.Tasks.Flared.Tunnel.Up do
  ...
  @doc false
  @spec build_command(String.t(), mode()) :: String.t()
  ...
  @doc false
  @spec build_env(mode()) :: %{String.t() => String.t()}
  def build_env({:remote, token}), do: %{"TUNNEL_TOKEN" => token}
  ...
  defp spawn_cloudflared(command, env) do
    case ElixirExec.run_link(command, env: env) do
