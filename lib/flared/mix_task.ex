defmodule Flared.MixTask do
  ...
  @doc false
  @spec build_up_env(up_mode()) :: %{String.t() => String.t()}
  def build_up_env({:remote, token}), do: %{"TUNNEL_TOKEN" => token}
  ...
  defp spawn_up(command, env) do
    case ElixirExec.run_link(command, env: env, stdout: :print, stderr: :print) do
      {:ok, %ElixirExec.Handle{controller: exec_pid, os_pid: os_pid}} ->
        {:ok, exec_pid, os_pid}
  ...
  defp spawn_with_config(executable, config_path) do
    case ElixirExec.run_link("#{executable} --config #{config_path} tunnel run",
           stdout: :print,
           stderr: :print
         ) do
      {:ok, %ElixirExec.Handle{controller: exec_pid, os_pid: os_pid}} ->
